...
public class AccountServiceImpl extends ServiceImpl<AccountMapper, Account> implements AccountService {
     ...
     * @return 操作结果，null表示正常，否则为错误原因
     */
    public String registerEmailAccount(RegisterByEmailVO info) {
        ...
        if (!this.save(account)) {
            ...
            detailsMapper.insert(new AccountDetails(account.getId()));
            return null;
        }
    }
     ...
     */
    @Override
    public String resetEmailAccountPassword(ResetPasswordByEmailVO info) {
        String verify = resetConfirm(new EmailConfirmVO(info.getEmail(), info.getCode()));
     ...
     */
    @Override
    public String resetConfirm(EmailConfirmVO info) {
        String email = info.getEmail();
        String code = this.getEmailVerifyCode(email);
        if (code == null) return "请先获取验证码";
        if (!code.equals(info.getCode())) return "验证码错误，请重新输入";
    ...
    /**
     * @description 更改邮箱验证操作，验证验证码是否正确，并在数据库中更改
     * @param id 用户ID
     * @param vo 邮箱以及验证码的封装对象
     * @return 操作结果，null表示正常，否则为错误原因
     */
    @Override
    public String modifyEmail(int id, EmailConfirmVO vo) {
    ...
    /**
     * @description 更改密码操作，验证原密码是否正确，并在数据库中更改
     * @param id 用户ID
     * @param vo 用户密码更改信息
     * @return 操作结果，null表示正常，否则为错误原因
     */
